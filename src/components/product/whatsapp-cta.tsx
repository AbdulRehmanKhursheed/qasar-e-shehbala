...
export function WhatsAppCTA({ productName, sku, fabricName, sizeName, productUrl, productId, variantId, className }: WhatsAppCTAProps) {
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLAnchorElement>) => {
      e.preventDefault();
      const ref = generateRef();
      const waUrl = buildWhatsAppUrl({ ref, productName, sku, fabric: fabricName, size: sizeName, productUrl });
  ...
  const fallbackUrl = buildWhatsAppUrl({ ref: "QES", productName, sku, fabric: fabricName, size: sizeName, productUrl });
  ...
  return (
    <a
      ...
      className={cn(
        ...
        "bg-[#25D366] px-6 py-3.5 text-[14px] font-semibold text-white",
        "shadow-sm transition-all duration-200 hover:bg-[#1da851] hover:shadow-md",
